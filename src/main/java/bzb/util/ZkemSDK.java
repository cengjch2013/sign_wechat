...
public class ZkemSDK {
	 ...
	 * @return
	 */
	public boolean readGeneralLogData(int ReadGeneralLogData) {
		boolean result = zkem.invoke("ReadGeneralLogData", ReadGeneralLogData).getBoolean();
		return result;
	}
	 ...
	 *         HH:mm:ss "VerifyMode" "InOutMode" "Year" 考勤时间：年 "Month" 考勤时间：月
	 *         "Day" 考勤时间：日 "Hour" 考勤时间：时 "Minute" 考勤时间：分 "Second" 考勤时间：秒
	 */
	public List<Map<String, Object>> getGeneralLogData(int dwMachineNumber) {
		Variant v0 = new Variant(dwMachineNumber);
		...
		do {
			...
			if (newresult) {
				...
				String time =  dwYear.getIntRef() + "-" + dwMonth.getIntRef() + "-" + dwDay.getIntRef() + " "
						+ dwHour.getIntRef() + ":" + dwMinute.getIntRef() + ":" + dwSecond.getIntRef();
				
				try {
					m.put("Time",formater.format(formater.parse(time)));
	 ...
	 */
	public List<Map<String, Object>> getUserInfo(int dwMachineNumber) {
		List<Map<String, Object>> resultList = new ArrayList<Map<String, Object>>();
		// 将用户数据读入缓存中。
		Variant v0 = new Variant(dwMachineNumber);
		boolean result = zkem.invoke("ReadAllUserID", v0).getBoolean();
	 ...
	 * @return
	 */
	public boolean setUserInfo(int dwMachineNumber, String number, String name, String password, int isPrivilege, boolean enabled) {
		Variant v0 = new Variant(dwMachineNumber);
	 ...
	 * @return
	 */
	public Map<String, Object> getUserInfoByNumber(String number, int dwMachineNumber) {
		Variant v0 = new Variant(dwMachineNumber);
		Variant sdwEnrollNumber = new Variant(number, true);
		Variant sName = new Variant("", true);
		Variant sPassword = new Variant("", true);
	...
	/**
	 * 清楚机器内所有考勤记录
	 * @param dwMachineNumber 机器号码
	 * @return
	 */
	public boolean clearGLog(int dwMachineNumber){
		return zkem.invoke("ClearGLog", dwMachineNumber).getBoolean();
	}
}
